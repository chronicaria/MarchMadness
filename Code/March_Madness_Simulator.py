...
start_date = "2025-02-26"  # Only simulate games on or after this date
simulations = 10000
...
for i in range(simulations):
    ...
    # Pairings: (Seed 1 vs. 16), (2 vs. 15), (3 vs. 14), (4 vs. 13),
    ...
    # Matchups:
    # Game 1: Winner of (1 vs. 16) vs. Winner of (8 vs. 9)
    # Game 2: Winner of (5 vs. 12) vs. Winner of (4 vs. 13)
    # Game 3: Winner of (6 vs. 11) vs. Winner of (3 vs. 14)
    # Game 4: Winner of (7 vs. 10) vs. Winner of (2 vs. 15)
    ...
    # --- Tournament Simulation: Round of 16 (Top 16) ---
    # Two games per region:
    # Game 1: Winner of Round32 Game 1 vs. Winner of Round32 Game 2
    # Game 2: Winner of Round32 Game 3 vs. Winner of Round32 Game 4
    ...
    # --- Tournament Simulation: Round of 8 (Top 8 / Regional Finals) ---
    # One game per region:
    # Winner of Round16 Game 1 vs. Winner of Round16 Game 2
    region_champions = []
    for region in range(4):
        winners16 = region_round16_winners[region]
        result = simulate_game(winners16[0], winners16[1], "N")
        champion = winners16[0] if result == 1 else winners16[1]
        region_champions.append(champion)
    
    # --- Semifinals (Final Four) ---
    # Assume region_champions order: [South, Midwest, West, East]
    ...
    # --- Update Tournament Progress Counts ---
    # t64: Every team in the initial bracket (64 teams)
    for region in range(4):
        for team in sim_bracket[region]:
            tournament_progress[team][0] += 1
    # t32: Winners from Round of 64 (32 teams)
    for region in range(4):
        for team in region_round64_winners[region]:
            tournament_progress[team][1] += 1
    # t16: Winners from Round of 32 (16 teams)
    for region in range(4):
        for team in region_round32_winners[region]:
            tournament_progress[team][2] += 1
    # t8: Winners from Round of 16 (8 teams)
    for region in range(4):
        for team in region_round16_winners[region]:
            tournament_progress[team][3] += 1
    # t4: Regional champions (4 teams)
    for team in region_champions:
        tournament_progress[team][4] += 1
    # t2: Semifinal winners (2 teams)
    tournament_progress[semi1_winner][5] += 1
    tournament_progress[semi2_winner][5] += 1
    # t1: National champion (1 team)
    tournament_progress[champion][6] += 1
